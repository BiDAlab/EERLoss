...
@tf.function
def calculate_eer_loss(y_true, y_pred, total_sets, binary_search_steps, beta, legitimate_mask, impostor_mask) -> tf.Tensor:
    ...
    tl = 100.0 * tf.math.pow(tl, 1.0/beta);
    ...
    tr = 100.0 * tf.math.pow(tr, 1.0/beta);
    retval = tl + tr;

    tf.print(tf.strings.format("[eerArea-keras] deer={}    far={}    frr={}    tl={}    tr={}    loss={}", [deer, far, frr, tl, tr, retval]), output_stream=sys.stdout);
    return retval;
...
class EERLoss(Loss):
    def __init__(self, beta, sets_per_batch, samples_per_set, binary_search_steps):
    ...
    def call(self, y_true, y_pred):
        return calculate_eer_loss(y_true, y_pred, self.sets_per_batch, self.binary_search_steps, self.beta, self.legitimate_mask, self.impostor_mask);



def get_loss(beta, sets_per_batch, samples_per_set, binary_search_steps):
    return EERLoss(beta, sets_per_batch, samples_per_set, binary_search_steps);
