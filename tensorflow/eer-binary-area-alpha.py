...
@tf.function
def calculate_eer_loss(y_true, y_pred, total_sets, binary_search_steps, alpha, legitimate_mask, impostor_mask, verbose) -> tf.Tensor:
    ...
    tl = tf.reduce_sum(tf.maximum(0.0, legitimate_scores - (1.0 - alpha) * deer));
    tl /= tf.cast(tf.shape(legitimate_scores)[0], dtype=tf.float32);
    tr = tf.reduce_sum(tf.maximum(0.0, (1.0 + alpha) * deer - impostor_scores));
...
class EERLoss(Loss):
    def __init__(self, total_sets, binary_search_steps):
        super(EERLoss, self).__init__()
        self.total_sets = total_sets;
        self.binary_search_steps = binary_search_steps;
        self.verbose = kvc_commandline.get_commandline_parameter_bool("verbose-loss");
        self.alpha = kvc_commandline.get_commandline_parameter_float("ALPHA", 0.0);
    ...
    def call(self, y_true, y_pred):
        return calculate_eer_loss(y_true, y_pred, self.total_sets, self.binary_search_steps, self.alpha, self.legitimate_mask, self.impostor_mask, self.verbose);
