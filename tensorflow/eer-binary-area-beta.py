...
@tf.function
def calculate_eer_loss(y_true, y_pred, total_sets, binary_search_steps, beta, legitimate_mask, impostor_mask, verbose) -> tf.Tensor:
    ...
    tl = tf.reduce_sum(tf.pow(tf.maximum(0.0000001, legitimate_scores - deer)/deer,beta));
    # tf.print(tf.strings.format("[eer-binary]    tl1={}", [tl]), output_stream=sys.stdout);
    tl /= tf.cast(tf.shape(legitimate_scores)[0], dtype=tf.float32);
    tl = tf.math.pow(tl, 1.0/beta);
    # tf.print(tf.strings.format("[eer-binary]    tl2={}", [tl]), output_stream=sys.stdout);
    tr = tf.reduce_sum(tf.pow(tf.maximum(0.0000001, deer - impostor_scores)/deer,beta));
    # tf.print(tf.strings.format("[eer-binary]    tr1={}", [tr]), output_stream=sys.stdout);
    tr /= tf.cast(tf.shape(impostor_scores)[0], dtype=tf.float32);
    tr = tf.math.pow(tr, 1.0/beta);
    # tf.print(tf.strings.format("[eer-binary]    tr2={}", [tr]), output_stream=sys.stdout);
    retval = 100.0 * (tl + tr);
    ...
    tf.print(tf.strings.format("[eer-binary-beta]    far={}    frr={}    tl={}    tr={}    loss={}", [far, frr, tl, tr, retval]), output_stream=sys.stdout);
    return retval;
...
class EERLoss(Loss):
    def __init__(self, total_sets, samples_per_set, binary_search_steps, beta):
    ...
    def call(self, y_true, y_pred):
        return calculate_eer_loss(y_true, y_pred, self.total_sets, self.binary_search_steps, self.beta, self.legitimate_mask, self.impostor_mask, self.verbose);
