...
@tf.function
def calculate_eer_loss(y_true, y_pred, total_sets, binary_search_steps, alpha, beta, legitimate_mask, impostor_mask, verbose) -> tf.Tensor:
    ...
    tl = tf.reduce_sum(tf.pow(tf.maximum(0.0000001, legitimate_scores - (1.0 - alpha) * deer)/deer,beta));
    ...
    tr = tf.reduce_sum(tf.pow(tf.maximum(0.0000001, (1.0 + alpha) * deer - impostor_scores)/deer,beta));
...
class EERLoss(Loss):
    def __init__(self, total_sets, samples_per_set, binary_search_steps, alpha, beta):
    ...
    def call(self, y_true, y_pred):
        return calculate_eer_loss(y_true, y_pred, self.total_sets, self.binary_search_steps, self.alpha, self.beta, self.legitimate_mask, self.impostor_mask, self.verbose);
